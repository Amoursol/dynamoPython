...
# We create a 'Filtered Element Collector' that will collect, in the 'Active View' of our document, all 
# 'Family Instances' of a particular category ('Planting'). We then cast it to Elements so we can use it in Dynamo.
builtInCollector = FilteredElementCollector( doc, doc.ActiveView.Id ).OfClass( FamilyInstance )
.OfCategory( BuiltInCategory.OST_Planting ).WhereElementIsNotElementType().ToElements()
