...
clr.AddReference("RevitServices") # Adding the RevitServices.dll special Dynamo module to 
# deal with Revit
...
# From the Autodesk namespace - derived down to the Revit Database, we imoprt only the 
# Filtered Element Collector, Wall, ElementId and ElementLevelFilter classes
from Autodesk.Revit.DB import FilteredElementCollector, Wall, ElementId, ElementLevelFilter 


# Here we give the Revit Document a nickname of 'doc' which allows us to simply call 'doc' 
# later without having to type the long namespace name 
...
# We then run a 'Filtered Element Collector' with our created 'filter' (Chosen Level), and 
# discount any element that is an 'Element Type' before using the 'ToElements' call to return real elements.
